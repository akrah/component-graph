	...
	ui(new Ui::MainWindow),coreFilter(0)
{
	...
	connect(ui->sliderAreaMax,SIGNAL(valueChanged(int)),this, SLOT(onFilteringValueChanged(int)));
	connect(ui->sliderAreaMin,SIGNAL(valueChanged(int)),this, SLOT(onFilteringValueChanged(int)));

	connect(ui->sliderContrastMax,SIGNAL(valueChanged(int)),this, SLOT(onFilteringValueChanged(int)));
	connect(ui->sliderContrastMax,SIGNAL(valueChanged(int)),ui->spinContrastMax, SLOT(setValue(int)));
	connect(ui->sliderContrastMin,SIGNAL(valueChanged(int)),this, SLOT(onFilteringValueChanged(int)));
	connect(ui->sliderContrastMin,SIGNAL(valueChanged(int)),ui->spinContrastMin, SLOT(setValue(int)));

	connect(ui->sliderAdaptiveArea,SIGNAL(valueChanged(int)),this, SLOT(onAdaptiveValueChanged(int)));
	connect(ui->sliderAdaptiveContrast,SIGNAL(valueChanged(int)),this, SLOT(onAdaptiveValueChanged(int)));
...
void MainWindow::loadImage(QString &filename)
{
	...
	if(qtImage.load(filename))
		...
		ui->sliderAreaMin->setMaximum(dx*dy+100);
		ui->spinAreaMin->setMaximum(dx*dy+100);
		ui->sliderAreaMax->setMaximum(dx*dy+1);
		ui->spinAreaMax->setMaximum(dx*dy+1);
		ui->sliderAreaMax->setValue(dx*dy);

		ui->sliderContrastMin->setMaximum(255*3);
		ui->spinContrastMin->setMaximum(255*3);
		ui->sliderContrastMax->setMaximum(255*3);
		ui->spinContrastMax->setMaximum(255*3);
		ui->sliderContrastMax->setValue(255*3);
...
{
	int areaMin=ui->sliderAreaMin->value();
	int areaMax=ui->sliderAreaMax->value();

	int contrastMin=ui->sliderContrastMin->value();
	int contrastMax=ui->sliderContrastMax->value();
...
//        }
		...
		int maximum = (coreFilter->imSource.getSizeX()/nx)*(coreFilter->imSource.getSizeY()/ny)+10;
		ui->sliderAreaMin->setMaximum(maximum);
		ui->spinAreaMin->setMaximum(maximum);
	}
}
...
void MainWindow::onAdaptiveValueChanged(int value)
{
	if(resultComputed) {
		...
		int p=ui->sliderAdaptiveArea->value();
		int q=ui->sliderAdaptiveContrast->value();
